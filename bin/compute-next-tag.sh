...
#
# Tags look like `v<Uptime Kuma version>-<release>`:
#
# - if defaults/main.yml points at an Uptime Kuma version that has never been
#   released, the release counter restarts at 0 (`v1.23.17-0`)
# - otherwise the counter is incremented (`v1.23.17-1`), but only if something
...
# Anchored at the start of the line so that neither the `# renovate:` annotation
# above it nor the variables derived from it (`uptime_kuma_container_image_tag`,
# which appends a flavor suffix) can be picked up instead. This has to be the
# literal value Renovate rewrites, not something Jinja assembles.
version="$(sed -nE 's|^uptime_kuma_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Uptime Kuma version from $defaults_path"
	exit 1
fi

# Tags carry a leading `v` which the version value itself does not, and the
# `#v` guards against a version value that already has one.
